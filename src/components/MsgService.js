...
export default function MsgService() {
  const { isMsgService, setIsMsgService, setShowServiceListModal } = useContext(
    GlobalContext
  );

  const setIsModalOpen = () => {
    setIsMsgService(null);
    setShowServiceListModal(true);
  };

  if (isMsgService) {
    const msg = isMsgService[0].msg;
    const screen = isMsgService[0].screen;

    return (
      <div
        className={`fixed z-50 top-0 left-0 w-full h-full overflow-auto bg-greensas bg-opacity-40 ${
          isMsgService ? "block" : "hidden"
        }`}
      >
